...
//定义多叉树基础模型
joint.shapes.btree.Model = joint.shapes.basic.Generic.extend(_.extend({}, joint.shapes.basic.PortsModelInterface, {
   ...
   */
  isLeaf: function () {
    return this instanceof joint.shapes.btree.Leaf;
  },
  ...
  isLocalLeaf: function () {
    return this instanceof joint.shapes.btree.Node && !this.hasAnyChild();
  },
  ...
  isLocalRoot: function () {
    return this instanceof joint.shapes.btree.Node && _.isUndefined(this.getParent())
  },
   ...
   */
  isNode: function () {
    return this instanceof joint.shapes.btree.Node;
  },
  /**
   * 返回是否是连线
   * @returns {boolean}
   */
  isLink: function () {
    return this instanceof joint.shapes.btree.Link;
  },
  ...
  hasLeftChild: function () {
    return !_.isUndefined(this.getLeftChild());
  },
  ...
  hasRightChild: function () {
    return !_.isUndefined(this.getRightChild());
  },
  ...
  getRoot: function () {
    ...
    if (!(greatestAncestor instanceof joint.shapes.btree.Root))
      return undefined;
   ...
   * @returns {*|Array}
   */
  getRightDescendants: function (callback) {
    var rightChild = this.getRightChild();
    if (_.isUndefined(rightChild))
      return [];
    return rightChild._recursiveGatherChild([rightChild], callback);
  },
  ...
  getLeftChild: function () {
    var leftChild, that = this;
    this.graph.getConnectedLinks(this, {outbound: true}).forEach(function (link) {
      if (link.get('source').port === 'out1') {
        leftChild = that.graph.getCell(link.get('target').id);
      }
    });
    return leftChild;
  },
  ...
  getRightChild: function () {
    var rightChild, that = this;
    this.graph.getConnectedLinks(this, {outbound: true}).forEach(function (link) {
      if (link.get('source').port === 'out2') {
        rightChild = that.graph.getCell(link.get('target').id);
      }
    });
    return rightChild;
  },
  ...
  getLeaves: function () {
    return this._recursiveGatherChild([], function (cell) {
      return cell instanceof joint.shapes.btree.Leaf;
    });
  },
  ...
  getLocalLeaves: function () {
    return this._recursiveGatherChild([], function (cell) {
      if (cell.hasAnyChild())
        return false;
      if (cell instanceof joint.shapes.btree.Leaf) return false;
      return true;
    });
  },
...
//有一个inPort和两个outPort
joint.shapes.btree.Node = joint.shapes.btree.Model.extend({
  ...
  }, joint.shapes.btree.Model.prototype.defaults)
});

//定制的二叉树连线：贝塞尔曲线
joint.shapes.btree.Link = joint.dia.Link.extend({
...
joint.shapes.btree.ModelView = joint.dia.ElementView.extend(joint.shapes.basic.PortsViewInterface);
joint.shapes.btree.NodeView = joint.shapes.btree.ModelView;
